...
echo "$0: generate alignment in $data using model from $srcdir, putting results in $dir"
...
$cmd JOB=1:$nj $dir/log/align_word.JOB.log \
  nbest-to-ctm "ark,t:$dir/aligned.JOB" "-" \| \
    utils/int2sym.pl -f 5 $lang/words.txt '>' "$dir/word.JOB.ctm"

$cmd JOB=1:$nj $dir/log/align_phone.JOB.log \
  lattice-to-phone-lattice "$dir/final.mdl" "ark,t:$dir/aligned.JOB" "ark:-" \| \
  nbest-to-ctm "ark:-" "-" \| \
    utils/int2sym.pl -f 5 $lang/phones.txt '>' "$dir/phone.JOB.ctm" || exit 1;

for n in $(seq $nj); do
  cat $dir/word.$n.ctm || exit 1;
done > $dir/word.ctm || exit 1;
