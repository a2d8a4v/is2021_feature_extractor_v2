...
class GOPModel(object):
    def __init__(self, data_dir, model_dir, dont_care_phone_id, use_sigmoid = True):
        # preprocess
        self.annotation_seq = self.__prepAnnotation(data_dir)
    ...
    def __prepAnnotation(self, data_dir):
        ''' Read and process (data related) '''
        # phone_text (phone_text) {'utt': ["p1", ..., "pn"]}
        # annotation_seq (annotation_seq) (ignore insertion) {'utt': [1,0]}
        phone_text = {}
        annotation_seq = {}
        with open(data_dir + "/annotation.txt", "r") as anno_file:
            for line in anno_file.readlines():
                line = line.split("\n")[0].split(" ")
                utt_id = line[0]
                phone_text[utt_id] = line[1].split(",")
                ann_seq = [ (t=="T") for t in line[2].split(",") ]
                #ann_seq = [t for t in line[1].split(",") ]
