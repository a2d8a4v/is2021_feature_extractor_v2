...
with open(args.wav_scp_file_path, "r") as fn:
...
with open(args.text_file_path, "r") as fn:
...
if args.long_decode_mode:
    if os.path.exists(os.path.join(data_dir, tmp_apl_decoding+".{}.list".format(split_number))):
        ...
        with open(os.path.join(data_dir, tmp_apl_decoding+".{}.list".format(split_number)), "r") as fn:
...
for i, utt_id in enumerate(tqdm(utt_list)):
    ...
    if args.long_decode_mode:
        ...
        with open(os.path.join(data_dir, tmp_apl_decoding+".{}.list".format(split_number)), "a") as fn:
...
# record error
if err_list:
    with open(output_dir + "/error.{}".format(split_number), "w") as fn:
...
with open(output_dir + "/ctm.{}".format(split_number), "w") as fn:
    end_time = -100000
    for utt_id in utt_list:
