...
if [ $stage -le 3 ]; then
  if [ $generate_ali_from_lats == "false" ]; then
    echo "WARNING: generate_ali_from_lats == $generate_ali_from_lats";
    echo "WARNING: This is OK if you know what you are doing.";
    sleep 5;
  else
    mkdir -p $dir/ctm_phone
    $cmd JOB=1:$nj $dir/log/ali2phone.JOB.log \
         ali-to-phones --frame-shift=0.03 --ctm-output $srcdir/${iter}.mdl \
            ark:"gunzip -c $dir/ali.JOB.gz|" - \| \
            utils/int2sym.pl -f 5 $lang/phones.txt '>' $dir/ctm_phone/ctm.JOB
         for n in `seq $nj`; do 
             cat $dir/ctm_phone/ctm.$n
         done > $dir/ctm_phone/ctm
  fi
fi
