...
if [ $stage -le 5 ] && [ $stop_stage -ge 5 ] ; then    
    for test_set in $test_sets; do
        ...
        mkdir -pv $data_dir/$queue_split

        python local.apl.v3/stt/split_input.py --split_number $nspk \
                                        --output_dir_path $data_dir/$queue_split \
                                        --wav_scp_file_path $data_dir/wav.scp \
    ...
    for test_set in $test_sets; do
        ...
        $cmd JOB=1:$nspk $logdir/prepare_feats.JOB.log \
            CUDA_VISIBLE_DEVICES=${CUDA} python local.apl.v3/stt/prepare_feats_queue.py --data_dir $data_dir \
                ...
                --gop_json_fn $data_dir/$queue_split/gop_scores.JOB.json \
                --utt2dur_file_path $data_dir/$queue_split/utt2dur.JOB \
                --lexicon $data_dir/$queue_split/lexicon.JOB.txt \
                --wav_scp_file_path $data_dir/$queue_split/wav.JOB.scp \
                --text_file_path $data_dir/$queue_split/$text.JOB || exit 1
